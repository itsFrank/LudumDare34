...
public class DoublePNGButton extends Button {
    ...
    public void draw(Color color) {
        ...
        if (isDowned) downed.draw(position.x, position.y, color);
        else {
            if (isMouseover) mouseover.draw(position.x, position.y, color);
            else idle.draw(position.x, position.y, color);
        }
    }
